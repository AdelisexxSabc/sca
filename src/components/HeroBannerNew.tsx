...
export default function HeroBanner({
  ...
  // 记录没有预告片的内容ID，避免重复请求
  const [noTrailerIds, setNoTrailerIds] = useState<Set<number | string>>(new Set());
  ...
  const refreshTrailerUrl = useCallback(async (doubanId: number | string) => {
    try {
      ...
      if (!response.ok) {
        ...
        // 标记为无预告片，避免重复请求
        setNoTrailerIds(prev => new Set(prev).add(doubanId));
        return null;
      }
  ...
  // 🎯 检查并刷新缺失的 trailer URL（组件挂载时，只执行一次）
  useEffect(() => {
    if (!items || items.length === 0) return;
    
    const checkAndRefreshMissingTrailers = async () => {
      for (const item of items) {
        // 如果有 douban_id 但没有 trailerUrl，且未标记为无预告片，尝试获取
        if (
          item.douban_id && 
          !item.trailerUrl && 
          !refreshedTrailerUrls[item.douban_id] &&
          !noTrailerIds.has(item.douban_id)
        ) {
  ...
  }, [items.length]); // 只在 items 数量变化时重新检查
  ...
  return (
    <div
      className="relative w-full h-[50vh] sm:h-[55vh] md:h-[60vh] overflow-hidden group rounded-xl"
                ...
                <video
                  ...
                  onError={async (e) => {
                    ...
                    if (item.douban_id) {
                      ...
                      // 重新刷新URL
                      const newUrl = await refreshTrailerUrl(doubanId);
                      if (newUrl) {
                        // 重新加载视频
                        video.load();
                      }
